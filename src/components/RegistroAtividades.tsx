      ...
      {filteredRecords.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredRecords.map((record) => (
            <div 
              key={record.id}
              className="bg-white rounded-2xl border border-slate-200/80 hover:border-indigo-200 shadow-sm p-6 flex flex-col justify-between transition-all hover:shadow-md group"
              aria-label={`Atividade: ${record.titulo_atividade}`}
            >
              {/* Card Top / Header */}
              <div>
                <div className="flex items-start justify-between gap-4 mb-4">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50/50 text-indigo-700 border border-indigo-100/50">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      ...
                      className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-rose-500 outline-hidden"
                ...
                {/* Title */}
                <h4 className="text-base font-bold text-slate-900 tracking-tight leading-snug mb-3 hover:text-indigo-600 transition-colors">
                ...
                <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs text-slate-500 mb-4 font-medium">
                  <span className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md border border-slate-100">
                    ...
                    Resp: <strong className="text-slate-700">{record.tecnico_responsavel || 'Não Identificado'}</strong>
                  </span>
                  {record.aocs_ci_vinculada && (
                    <span className="bg-slate-50 px-2 py-1 rounded-md border border-slate-100 font-mono text-[10px]">
              ...
              {/* Card Footer Actions */}
              <div className="border-t border-slate-100 pt-5 flex flex-col sm:flex-row gap-3 mt-auto">
                <button
                  onClick={() => handleViewReportClick(record)}
                  className="flex-1 py-2 px-3 border border-slate-200 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700 text-slate-700 text-xs font-semibold rounded-xl flex items-center justify-center gap-1.5 transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 outline-hidden"
                ...
                {record.link_drive_midias ? (
                  <a
                    ...
                    className="flex-1 py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl flex items-center justify-center gap-1.5 transition-colors focus-visible:ring-2 focus-visible:ring-emerald-500 outline-hidden shadow-xs"
                  ...
                  <button
                    disabled
                    className="flex-1 py-2 px-3 bg-slate-50 text-slate-400 text-xs font-semibold rounded-xl flex items-center justify-center gap-1.5 cursor-not-allowed border border-slate-100"
