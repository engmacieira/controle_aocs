...
import { CiRecord, SortOrder } from '../types';
import { ArrowUpDown, Edit3, Trash2, Search, Plus, Sparkles, FileText, Link, HelpCircle } from 'lucide-react';
...
export function CiTable({ records, onEdit, onDelete, onBulkDelete, onAdd }: CiTableProps) {
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-slate-100 pt-4 text-xs text-slate-500 font-medium font-mono">
          <span>
            ...
            {Math.min(filteredRecords.length, currentPage * pageSize)} de {filteredRecords.length} lançamentos de CIs
          </span>
          <div className="flex items-center gap-2">
            <button
